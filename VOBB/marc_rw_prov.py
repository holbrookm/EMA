...
def NGINPort(sub):
    ...
    r= requests.put(eirin04_put_request, auth =('admin', 'password'),  data = json_data, headers = headers)
...
def verifyPort(sub):
    ...
    query_string = str(eirin04_query_request + str(sub))
...
def check_existing_rw_subs(sub_no):
    """
    """    
...
def create_remote_worker(sub, pw):
    """ 
        Create the hosted office subscription.
    """    
    s1 = ims.remoteWorker(sub)
    session = {}
    session['emaSession'] = ema.emaLogin()
...
def main():    
    ...
    if check_ngin_access(subdata):
        ...
        for row in checked_subdata:
            sub = str('+' + str(row[0]))
            print ("Checking for Remote Workers subscription {0}".format(sub)) 
            r = check_existing_rw_subs(sub)
            sleep(5.25)
            if r.status_code == 200:
                existing_rw.append(row[0])
                ...
                action = create_remote_worker(sub, row[1])
                sleep(.25)
                #action = check_existing_rw_subs(sub) # Debug code only Please comment out for live action
                ...
                if action.status_code == 200:
                    ...
                    result_of_port = NGINPort(row[0])   #Remove for DEBUG 
                    if result_of_port.status_code == 200 or result_of_port.status_code == 201:
                        print (result_of_port.status_code)
        ...
        print("There are {0} existing HO subscriptions!".format(existing_rw.__len__()))
        print (existing_rw)
